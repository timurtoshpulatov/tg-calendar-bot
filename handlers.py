...
# ─── Inline-кнопки: подтверждение выполнения ────────────────────
...
# ─── Inline-кнопки: подтверждение удаления ──────────────────────
...
# ─── Inline-кнопки: выбор задачи для завершения ─────────────────
...
# ─── Inline-кнопки: выбор задачи для удаления ───────────────────
...
async def handle_text(message: types.Message):
    ...
    if parsed.intent == "complete_task":
        ...
        if not query:
            # Нет запроса для поиска — покажем ближайшую задачу
        ...
        if len(tasks) == 1:
            ...
            buttons = []
            for t in tasks[:5]:  # Ограничиваем 5 вариантами
                buttons.append([
